...
if __name__ == '__main__':
    ...
    # load weights:
    classifier.load_state_dict(torch.load("your_weight"))
    ...
    for i,data in enumerate(testdataloader, 0):
        ...
        write_points( os.path.join(opt.outf, str(0)+".ply"), points.numpy()[0,...], color_vis_feat[0,...], feat_value[0,...])
        write_points( os.path.join(opt.outf, str(1)+".ply"), points.numpy()[1,...], color_vis_feat[1,...], feat_value[1,...])
        write_points( os.path.join(opt.outf, str(2)+".ply"), points.numpy()[2,...], color_vis_feat[2,...], feat_value[2,...])
        write_points( os.path.join(opt.outf, str(3)+".ply"), points.numpy()[3,...], color_vis_feat[3,...], feat_value[3,...])
        write_points( os.path.join(opt.outf, str(4)+".ply"), points.numpy()[4,...], color_vis_feat[4,...], feat_value[4,...])
